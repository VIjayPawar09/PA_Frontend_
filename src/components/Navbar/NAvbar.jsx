...
const Navbar = () => {
  ...
  const handleLogin = async (e) => {
    ...
    // );
       try{
         const res=await axios.post("http://localhost:5000/user/login",credentials)
              ...
              {currentUser ? (
                <div className="flex items-center space-x-4">
                  <div className="bg-white text-indigo-600 px-3 py-1 rounded-full font-medium">
                    {currentUser.username} {currentUser.userType === "assistant" ? "(Assistant)" : "(Customer)"}
                  </div>
                  <button
            ...
            {currentUser ? (
              <>
                <div className="bg-white text-indigo-600 px-3 py-1 rounded-full font-medium inline-block my-2">
                  {currentUser.email} {currentUser.userType === "assistant" ? "(Assistant)" : "(Customer)"}
                </div>
                <button
                  onClick={handleLogout}
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleLogin}>
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className={`w-full p-3 rounded-lg font-medium transition-all duration-300 bg-blue-500 hover:bg-blue-600 text-white ${isLoading ? "opacity-70 cursor-not-allowed" : ""}`}
                >
                  {isLoading ? (
